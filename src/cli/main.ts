...
import {
  verifyAuth,
  type WardenAuthenticationError,
...
function writeEmptyRunLog(
  ...
  try {
    ...
    console.warn(
      `Warning: Failed to write run log: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  if (opts?.outputPath) {
    try {
      ...
      console.warn(
        `Warning: Failed to write output file: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  for (const result of results) {
    if (result.report) {
      ...
      if (result.failOn && shouldFail(reportForFail, result.failOn)) {
        ...
        failureReasons.push(
          `${result.name}: ${count} ${result.failOn}+ severity ${pluralize(count, "issue")}`,
        );
      }
    }
  }
...
async function outputResultsAndHandleFixes(
  ...
  try {
    ...
    reporter.warning(
      `Failed to write run log: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  if (options.json) {
    ...
    console.log(
      renderTerminalReport(filteredReports, reporter.mode, {
...
async function runSkills(
  ...
  if (!apiKey) {
    reporter.debug(`No API key found for ${providerName}. Using default auth.`);
  }
  ...
  try {
    ...
    if (options.json) {
      const { content } = writeEmptyRunLog(effectiveRepo, {
        traceId: getTraceId(),
        outputPath: options.output,
      });
      process.stdout.write(content);
    } else {
      writeEmptyRunLog(effectiveRepo, {
        traceId: getTraceId(),
        outputPath: options.output,
      });
    }
  ...
  if (options.skill) {
    ...
    const match = config
      ? resolveSkillConfigs(config, options.model).find(
          (t) => t.skill === options.skill,
        )
      : undefined;
  ...
  if (skillsToRun.length === 0) {
    ...
    if (options.json) {
      ...
      reporter.tip(
        "Specify a skill explicitly: warden <target> --skill <name>",
      );
    }
    return 0;
  }
  ...
  const sdkModel =
    config?.defaults?.model ?? options.model ?? process.env["WARDEN_MODEL"];
  ...
  const tasks: SkillTaskOptions[] = skillsToRun.map(
    ({ skill, remote, filters }) => ({
      ...
      resolveSkill: () =>
        resolveSkillAsync(skill, repoPath, {
          remote,
  ...
  const effectiveMinConfidence =
    options.minConfidence ?? config?.defaults?.minConfidence ?? "medium";
  const processed = processTaskResults(
    results,
    options.reportOn,
    effectiveMinConfidence,
  );
  return outputResultsAndHandleFixes(
    processed,
    options,
    reporter,
    repoPath ?? cwd,
    totalDuration,
    failFastController?.signal.aborted,
    logModel,
  );
}
...
async function runFileMode(
  ...
  if (pullRequest.files.length === 0) {
    if (options.json) {
      const { content } = writeEmptyRunLog(cwd, {
        traceId: getTraceId(),
        outputPath: options.output,
      });
      process.stdout.write(content);
    } else {
      writeEmptyRunLog(cwd, {
        traceId: getTraceId(),
        outputPath: options.output,
      });
      reporter.blank();
  ...
  reporter.success(
    `Found ${pullRequest.files.length} ${pluralize(pullRequest.files.length, "file")}`,
  );
...
async function runGitRefMode(
  ...
  if (pullRequest.files.length === 0) {
    if (options.json) {
      const { content } = writeEmptyRunLog(repoPath, {
        traceId: getTraceId(),
        outputPath: options.output,
      });
      process.stdout.write(content);
    } else {
      writeEmptyRunLog(repoPath, {
        traceId: getTraceId(),
        outputPath: options.output,
      });
...
async function runConfigMode(
  ...
  if (!existsSync(configPath)) {
    ...
    reporter.tip(
      "Create a warden.toml or specify targets: warden <files> --skill <name>",
    );
    return 1;
  }
  ...
  const providerName: ProviderName =
    (config.defaults?.provider?.name as ProviderName) ?? "openai";
  const provider = createProvider(providerName);
  ...
  const tasks: SkillTaskOptions[] = triggersToRun.map((trigger) => ({
    ...
    resolveSkill: () =>
      resolveSkillAsync(trigger.skill, repoPath, {
  ...
  const concurrency =
    options.parallel ?? config.runner?.concurrency ?? DEFAULT_CONCURRENCY;
  failFastController = options.failFast ? new AbortController() : undefined;
  const taskOptions = {
    mode: reporter.mode,
    verbosity: reporter.verbosity,
 ...
 */
async function runDirectSkillMode(
  options: CLIOptions,
  reporter: Reporter,
): Promise<number> {
  const cwd = process.cwd();
  ...
  if (pullRequest.files.length === 0) {
    if (options.json) {
      ...
      if (options.staged) {
        ...
        const tip =
          "Specify a git ref to analyze committed changes: warden main --skill <name>";
        reporter.renderEmptyState("No uncommitted changes found", tip);
      }
      reporter.blank();
    }
...
async function runCommand(
  ...
  // Classify targets
  const { gitRefs, filePatterns } = classifyTargets(targets, {
    forceGit: options.git,
  });
...
export async function main(): Promise<void> {
  ...
  try {
    ...
    const cfg = existsSync(cfgPath)
      ? loadWardenConfig(dirname(cfgPath))
      : undefined;
