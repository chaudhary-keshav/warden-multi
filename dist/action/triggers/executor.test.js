...
describe('executeTrigger', () => {
    ...
    it('handles skill resolution failure', async () => {
        ...
        expect(failSkillCheck).toHaveBeenCalledWith(mockOctokit, 123, expect.objectContaining({ message: 'Skill not found' }), { owner: 'test-owner', repo: 'test-repo', headSha: 'abc123' });
    });
    it('handles skill execution failure', async () => {
        ...
        expect(result.error).toBeDefined();
        expect(result.report).toBeUndefined();
        expect(failSkillCheck).toHaveBeenCalledWith(mockOctokit, 123, expect.objectContaining({ message: 'API error' }), { owner: 'test-owner', repo: 'test-repo', headSha: 'abc123' });
    });
