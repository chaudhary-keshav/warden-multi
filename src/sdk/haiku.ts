...
export function setGenAiResponseAttrs(
  ...
  span.setAttribute(
    ...
    totalInput + usage.output_tokens,
  );
  if (stopReason) {
...
function inferPrefill(schema: z.ZodType): string | undefined {
  ...
  if (
    "_def" in schema &&
    (schema as { _def: { typeName?: string } })._def.typeName === "ZodArray"
  )
    return "[";
  return undefined;
}
...
export async function callHaiku<T>(
  options: CallHaikuOptions<T>,
): Promise<HaikuResult<T>> {
  const {
    apiKey,
    prompt,
    schema,
 ...
 */
export async function callHaikuWithTools<T>(
  options: CallHaikuWithToolsOptions<T>,
): Promise<HaikuResult<T>> {
  const {
    apiKey,
    prompt,
      ...
      for (let iteration = 0; iteration < maxIterations; iteration++) {
        ...
        if (response.stop_reason === "tool_use") {
          ...
          if (toolUseBlocks.length === 0) {
            return {
              success: false,
              error: "Tool use indicated but no tool calls found",
...
/**
 * Provider-aware auxiliary call.
 *
 * When a non-Claude provider is set, uses provider.auxiliaryQuery() instead
 * of the Anthropic Messages API. Falls back to callHaiku() for Claude.
 */
export async function callAuxiliary<T>(
  options: CallHaikuOptions<T> & {
    provider?: import("../providers/types.js").LLMProvider;
  },
): Promise<HaikuResult<T>> {
  const { provider, ...haikuOpts } = options;

  // Claude provider (or no provider) uses the original callHaiku path
  if (!provider || provider.name === "claude") {
    return callHaiku(haikuOpts);
  }

  // Non-Claude provider: use auxiliaryQuery
  const result = await provider.auxiliaryQuery({
    systemPrompt:
      "You are a JSON-only assistant. Respond only with valid JSON matching the requested schema.",
  ...
  if (!result.success) {
    return {
      success: false,
      error: result.error ?? "Provider auxiliary query failed",
      usage: result.usage,
    };
  }
