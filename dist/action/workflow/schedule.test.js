...
vi.mock("./base.js", async () => {
    ...
    const mockedSetFailed = vi.fn((msg) => {
    ...
    return {
        ...
        handleTriggerErrors: (triggerErrors, totalTriggers) => {
...
vi.mock("../../skills/loader.js", async () => {
    ...
    return {
        ...actual,
        resolveSkillAsync: vi.fn(() => Promise.resolve({
            name: "test-skill",
...
describe("runScheduleWorkflow", () => {
    ...
    describe("configuration and early exit", () => {
        it("exits cleanly when warden.toml is missing", async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith("::warning::No warden.toml found. Skipping analysis.");
        });
        ...
        it("fails when GITHUB_REPOSITORY is not set", async () => {
            delete process.env["GITHUB_REPOSITORY"];
            await expect(runScheduleWorkflow(mockOctokit, createDefaultInputs(), SCHEDULE_FIXTURES)).rejects.toThrow("setFailed");
            expect(mockSetFailed).toHaveBeenCalledWith("GITHUB_REPOSITORY environment variable not set");
        });
        ...
        it("fails when GITHUB_SHA is not set", async () => {
            delete process.env["GITHUB_SHA"];
            await expect(runScheduleWorkflow(mockOctokit, createDefaultInputs(), SCHEDULE_FIXTURES)).rejects.toThrow("setFailed");
            expect(mockSetFailed).toHaveBeenCalledWith("GITHUB_SHA environment variable not set");
        });
    });
    // ---------------------------------------------------------------------------
    // Happy Path
    ...
    describe("happy path", () => {
        it("runs skill and creates issue when findings exist", async () => {
            ...
            expect(mockCreateOrUpdateIssue).toHaveBeenCalledWith(mockOctokit, "test-owner", "test-repo", [report], expect.objectContaining({
    ...
    describe("issue and PR creation", () => {
        it("creates fix PR when schedule.createFixPR is enabled", async () => {
            ...
            await runScheduleWorkflow(mockOctokit, createDefaultInputs(), SCHEDULE_FIXPR_FIXTURES);
            expect(mockCreateFixPR).toHaveBeenCalledWith(mockOctokit, "test-owner", "test-repo", [finding], expect.objectContaining({
        ...
        it("uses custom issue title from schedule config", async () => {
            ...
            mockRunSkill.mockResolvedValue(report);
            await runScheduleWorkflow(mockOctokit, createDefaultInputs(), SCHEDULE_TITLE_FIXTURES);
            expect(mockCreateOrUpdateIssue).toHaveBeenCalledWith(mockOctokit, "test-owner", "test-repo", [report], expect.objectContaining({
    ...
    describe("failure and error handling", () => {
        it("fails when failOn threshold is met and failCheck is true", async () => {
            const finding = createFinding({ severity: "high" });
            mockRunSkill.mockResolvedValue(createSkillReport({ findings: [finding] }));
            await expect(runScheduleWorkflow(mockOctokit, createDefaultInputs({ failOn: "high", failCheck: true }), SCHEDULE_FIXTURES)).rejects.toThrow("setFailed");
        ...
        it("does not fail when failOn threshold is met but failCheck is false", async () => {
            ...
            await runScheduleWorkflow(mockOctokit, createDefaultInputs({ failOn: "high" }), SCHEDULE_FIXTURES);
            expect(mockSetFailed).not.toHaveBeenCalled();
        });
        it("records error and calls handleTriggerErrors when trigger throws", async () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining("Trigger test-skill-a failed"));
        });
        it("fails when all triggers throw", async () => {
            ...
            await expect(runScheduleWorkflow(mockOctokit, createDefaultInputs(), SCHEDULE_MULTI_FIXTURES)).rejects.toThrow("setFailed");
            expect(mockSetFailed).toHaveBeenCalledWith(expect.stringContaining("All 2 trigger(s) failed"));
        });
    });
    // ---------------------------------------------------------------------------
    // Outputs
    ...
    describe("outputs", () => {
        it("aggregates severity counts across multiple triggers", async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("2 total findings"));
        });
    });
});
