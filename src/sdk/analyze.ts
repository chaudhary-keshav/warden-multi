...
async function parseHunkOutput(
  ...
  const fallback = await extractFindingsWithLLM(
    result.result,
    apiKey,
    auxiliaryMaxRetries,
  );
...
/**
 * Execute a query using a non-Claude provider (OpenAI, Gemini).
 * Converts the provider's LLMQueryResult into a QueryExecutionResult
 * with a synthetic SDKResultMessage for downstream compatibility.
 */
async function executeQueryWithProvider(
  systemPrompt: string,
  userPrompt: string,
  repoPath: string,
  ...
  if (!queryResult.success) {
    return { authError: queryResult.error };
  }

  // Create a synthetic SDKResultMessage so the downstream parsing pipeline
  // (parseHunkOutput, extractFindingsJson, etc.) works unchanged.
  // We use a type assertion because non-Claude providers don't have the full
  // Claude SDK metadata (modelUsage, permission_denials, uuid, etc.).
  const syntheticResult = {
    ...
    session_id: "",
    total_cost_usd: queryResult.costUSD ?? 0,
    usage: {
      input_tokens: queryResult.usage.inputTokens,
      output_tokens: queryResult.usage.outputTokens,
      cache_read_input_tokens: queryResult.usage.cacheReadInputTokens ?? 0,
      cache_creation_input_tokens:
        queryResult.usage.cacheCreationInputTokens ?? 0,
      cache_creation: {
    ...
    modelUsage: {},
    permission_denials: [],
    uuid: "" as SDKResultMessage extends { uuid: infer U } ? U : string,
    duration_ms: 0,
    duration_api_ms: 0,
    num_turns: 0,
...
async function executeQuery(
  ...
  // Dispatch to non-Claude providers when configured
  if (options.provider && options.provider.name !== "claude") {
    return executeQueryWithProvider(
      systemPrompt,
      userPrompt,
      repoPath,
      options,
  ...
  return Sentry.startSpan(
    ...
    async (span) => {
      ...
      for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
        ...
        try {
          const { result: resultMessage, authError } = await executeQuery(
            systemPrompt,
            userPrompt,
            repoPath,
            options,
          ...
          const isError =
            resultMessage.is_error || resultMessage.subtype !== "success";
          ...
          if (isError) {
            ...
            if (callbacks?.onHunkFailed) {
              callbacks.onHunkFailed(
                callbacks.lineRange,
                `SDK execution failed: ${errorSummary}`,
              );
            } else {
          ...
          const { filtered: filteredFindings, dropped } =
          ...
          try {
            ...
            if (callbacks?.onHunkFailed) {
              callbacks.onHunkFailed(
                callbacks.lineRange,
                "Analysis aborted during retry delay",
              );
            }
            return {
              findings: [],
      ...
      if (lastError) {
        if (callbacks?.onHunkFailed) {
          callbacks.onHunkFailed(
            callbacks.lineRange,
            `All retry attempts failed: ${finalError}`,
          );
        } else {
 ...
 */
function attachElapsedTime(
  findings: Finding[],
  skillStartTime: number | undefined,
): void {
...
export async function runSkill(
  ...
  if (!context.pullRequest) {
    throw new SkillRunnerError(
      "Pull request context required for skill execution",
    );
  }
  ...
  const totalHunks = fileHunks.reduce(
    (sum, file) => sum + file.hunks.length,
    0,
  );
  ...
  const prContext: PRPromptContext = {
    changedFiles: isPullRequest
      ? context.pullRequest.files.map((f) => f.filename)
      : [],
