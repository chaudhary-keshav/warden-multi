 ...
 */
export async function evaluateFixAttempts(octokit, comments, context, currentFindings, apiKey, maxRetries) {
    return Sentry.startSpan({
        ...
        for (const comment of commentsToEvaluate) {
            ...
            const evalResultData = await Sentry.startSpan({
                ...
                const evalResult = await evaluateFix({ comment, changedFiles, codeBeforeFix, codeAfterFix, commitMessages }, toolContext, apiKey, maxRetries);
            ...
            let finalVerdict = evalResult.verdict.status;
            if (reDetected) {
                ...
                result.toReply.push({
                    comment,
                    replyBody: formatFailedFixReply(context.headSha, 'The fix attempt was made, but the same issue was detected again in the updated code.'),
        ...
        emitFixEvalMetrics(result.evaluated, result.toResolve.length, result.failedEvaluations, result.skipped, result.uniqueFindingsEvaluated, result.uniqueFindingsCodeChanged, result.uniqueFindingsResolved);
        return result;
    });
}
