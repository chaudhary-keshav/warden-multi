...
vi.mock('./github.js', () => ({
    ...
    formatFailedFixReply: vi.fn((sha, reasoning) => {
...
describe('evaluateFixAttempts', () => {
    ...
    it('overrides resolved verdict when issue is re-detected', async () => {
        ...
        const result = await evaluateFixAttempts(mockOctokit, [comment], defaultContext, [finding], 'api-key');
    ...
    it('limits evaluation to MAX_EVALUATIONS (20)', async () => {
        const comments = Array.from({ length: 25 }, (_, i) => createComment({ id: i + 1, threadId: `thread-${i}` }));
        mockEvaluateFix.mockResolvedValue({
...
//# sourceMappingURL=fix-evaluation.test.js.map