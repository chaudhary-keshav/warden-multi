...
export interface ActionInputs {
  ...
  provider: string;
  /** API key for Anthropic API (empty if using OAuth or other provider) */
...
export function parseActionInputs(): ActionInputs {
  ...
  if (provider === "openai") {
    ...
    const openaiKey =
      getInput("openai-api-key") || process.env["OPENAI_API_KEY"] || "";
    if (!openaiKey) {
      throw new Error(
        "OpenAI API key not found. Provide via openai-api-key input or OPENAI_API_KEY env var.",
      );
    }
    ...
    const geminiKey =
      getInput("gemini-api-key") || process.env["GEMINI_API_KEY"] || "";
    if (!geminiKey) {
      throw new Error(
        "Gemini API key not found. Provide via gemini-api-key input or GEMINI_API_KEY env var.",
      );
    }
  ...
  const parallelParsed = parseInt(
    getInput("parallel") || String(DEFAULT_CONCURRENCY),
    10,
  );
  ...
  return {
    ...
    requestChanges,
    failCheck,
    parallel: Number.isNaN(parallelParsed)
      ? DEFAULT_CONCURRENCY
      : parallelParsed,
  };
}
