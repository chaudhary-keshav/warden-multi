...
describe('postTriggerReview', () => {
    ...
    it('deduplicates findings against existing comments', async () => {
        ...
        expect(processDuplicateActions).toHaveBeenCalledWith(mockOctokit, 'test-owner', 'test-repo', [{ type: 'react_external', finding, existingComment, matchType: 'hash' }], 'test-skill');
    ...
    it('posts REQUEST_CHANGES when all findings deduplicated but failOn threshold met', async () => {
        ...
        expect(processDuplicateActions).toHaveBeenCalledWith(mockOctokit, 'test-owner', 'test-repo', [{ type: 'update_warden', finding, existingComment, matchType: 'hash' }], 'test-skill');
    ...
    it('consolidates batch findings before dedup when multiple findings exist', async () => {
        ...
        expect(consolidateBatchFindings).toHaveBeenCalledWith([finding1, finding2], { apiKey: 'test-key', hashOnly: false });
        expect(postResult.posted).toBe(true);
